...
class SERPPreviewTab(QWidget):
    ...
    def generate_preview(self):
        ...
        if len(self.title_edit.text()) > 60:
            warnings.append(f"⚠️ Title too long: {len(self.title_edit.text())} chars (max 60)")
        elif len(self.title_edit.text()) < 50:
            warnings.append(f"⚠️ Title too short: {len(self.title_edit.text())} chars (recommend 50-60)")
        
        if len(self.desc_edit.toPlainText()) > 160:
            warnings.append(f"⚠️ Description too long: {len(self.desc_edit.toPlainText())} chars (max 160)")
        elif len(self.desc_edit.toPlainText()) < 120:
            warnings.append(f"⚠️ Description too short: {len(self.desc_edit.toPlainText())} chars (recommend 150-160)")
