...
class SpellCheckerTab(QWidget):
    ...
    def init_ui(self):
        ...
        if not HAS_SPELLCHECKER:
            warning = QLabel("⚠️ pyspellchecker not installed. Run: pip install pyspellchecker")
    ...
    def scan_spelling(self):
        ...
        if error_count == 0:
            ...
            self.summary_label.setText(f"⚠️ Found {error_count} potential spelling errors across {len(html_files)} files")
